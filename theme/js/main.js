            ...
            if ($('.news').length) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        var linkElement = document.querySelector("#noticia_imagem a")

                        // Adiciona o atributo alt no link
                        if (linkElement) {
                            linkElement.setAttribute("title", "Link do blog"); 
                        } else {

                        }
                        // Verifique se o elemento img está presente
                        var imgElement = document.querySelector("#noticia_imagem img");
                        // Adiciona os atributos width, height , alt e title
                        if (imgElement) {
                            imgElement.setAttribute("width", "240");  // Defina o valor desejado para a largura
                            imgElement.setAttribute("height", "240"); // Defina o valor desejado para a altura
                            imgElement.setAttribute("alt", "Imagem do blog"); // Defina o valor desejado para o alt
                            imgElement.setAttribute("title", "Imagem do blog"); // 
                        } else {
                        }
                    },
                });
...
const btn_continuar_comprando = document.querySelector('.btn-continuar-comprando');

if(btn_continuar_comprando){
    btn_continuar_comprando.addEventListener('click', () =>{
        const cart_container = document.querySelector('.cart-header');
        cart_container.classList.remove('active');
    });
}
...
//Régua de Beneficio Swiper
document.addEventListener('DOMContentLoaded', (event) => {
    if(document.querySelector(".regua-icons")){
        var totalIcons = 5;

        if(window.totalIcons){
            totalIcons = parseInt(window.totalIcons.total) || 0;
        }

        if(window.screen.width <= 700){
                const div_swiper = document.querySelector(".regua-icons");
                div_swiper.classList.add("swiper-regua");
                new Swiper('.swiper-regua', {
                    loop: true,
                    speed: 500,
                    autoplay: {
                        delay: 1500, 
                    ...
                    navigation: {
                        nextEl: '.swiper-regua .swiper-button-next',
                        prevEl: '.swiper-regua .swiper-button-prev',
                    },
                    breakpoints: {
                        320: { slidesPerView: 1 },
                        480: { slidesPerView: 1 },
                        700: { slidesPerView: totalIcons - 2 },
                    },
                    scrollbar: { el: '.swiper-scrollbar' },
                });
                } else {
                new Swiper('.regua-icons', {
                    breakpoints: {
                        900: { slidesPerView: totalIcons - 2 },
                        1200: { slidesPerView: totalIcons },
                        1980: { slidesPerView: totalIcons },
                    },
                });
        }
    }
...
document.addEventListener('DOMContentLoaded', (event) => {
    if(document.querySelector("#loja-protegida-img img")){
        // Verifique se o elemento img está presente
        const imgSeloSeguranca = document.querySelector("#loja-protegida-img img");
        // Adiciona os atributos width, height , alt e title
        if (imgSeloSeguranca) {
            imgSeloSeguranca.setAttribute("width", "130px");
            imgSeloSeguranca.setAttribute("height", "38px"); 
            imgSeloSeguranca.setAttribute("alt", "Imagem do blog"); 
            imgSeloSeguranca.setAttribute("loading", "lazy"); 
        } else {
        }
    }
});
...
function copyCode() {
    ...
    const coupon_copy_btn = document.getElementById("coupon-copy-btn");

    if (couponCode) {
        navigator.clipboard.writeText(couponCode).then(() => {
            coupon_copy_btn.innerText = 'COPIADO';

            setTimeout(() => {
                coupon_copy_btn.innerText = 'COPIAR';
            }, 2000);
        }, () => {
...
document.addEventListener("DOMContentLoaded", function() {
    const btn_open_coupon = document.getElementById("coupon-open");
    const coupon_copy_btn = document.getElementById("coupon-copy-btn");

    if (btn_open_coupon) {
        btn_open_coupon.addEventListener("click", () => {
            const container_coupon = document.querySelector(".coupon-container");
            const coupon_details = document.querySelector(".coupon-details");

            container_coupon.classList.toggle("active");
            coupon_details.classList.toggle("active");
        });
    }

    if(coupon_copy_btn){
        coupon_copy_btn.addEventListener("click", () => copyCode());
    }

    if(couponCodeElement){
        couponCodeElement.addEventListener('click', ()=> copyCode());
    }
